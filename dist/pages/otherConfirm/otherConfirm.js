...
Page({
    ...
    var ocbj = {
      ...
      success: function success(res) {
        ...
        if (res.data.data.photos.length !== 0) {
          ...
          res.data.data.photos.split(',').forEach(function (v) {
            photos.push({
              photo_url: v,
              is_ta: 1
            });
  ...
  // 行业遮罩
  chooseIndustryhy: function chooseIndustryhy() {
    this.setData({
      industryShow: true
    });
  },
  ...
  // 智能修正弹窗
  getFixInfoModal: function getFixInfoModal() {
    var that = this;
    wx.showModal({
      title: '智能修正',
  ...
  // 智能修正
  getFixInfo: function getFixInfo() {
    var that = this;
    var gfiObj = {
      url: useUrl.getUserInfoBySelf,
      data: {
      ...
      success: function success(res) {
        ...
        var job2 = mine.job.split('-');
        var v11 = that.data.industryOne.indexOf(job2[0]);
        var v22 = 0;
        if (v11 < 24) {
  ...
  // 获取我的相册图片数量
  getMyPhoto: function getMyPhoto() {
    var that = this;
    var gmp = {
      url: useUrl.myPhotos,
      data: {
...
//# sourceMappingURL=otherConfirm.js.map
